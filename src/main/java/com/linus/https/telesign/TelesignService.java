...
import com.linus.https.DefaultSSLSocketFactory;
...
public class TelesignService {
	...
	private static final String CALL_SUB_URL = "/v1/verify/call";
	private static final String SMS_URL = TELESIGN_BASE_URL + SMS_SUB_URL;
	private static final String CALL_URL = TELESIGN_BASE_URL + CALL_SUB_URL;
	...
	public static TelesignResult sendSMS(boolean useProxy, String countryCode, String phoneNumber, String verifyCode,
		...
		try {
			...
			StringEntity requestEntity = new StringEntity("phone_number=" + phoneNumber + "&language=" + language
			...
			String statusCode = String.valueOf(jsonStatus.getInt("code"));
	...
	public static TelesignResult sendCall(boolean useProxy, String countryCode, String phoneNumber, String verifyCode, String language) {
        TelesignResult result = new TelesignResult();
        ...
        try {
            ...
            httpPost.setEntity(new StringEntity("phone_number="+phoneNumber+"&language="+language+"&verify_code="+verifyCode));
            ...
            result.setStatusCode(statusCode);
            if (statusCode.equals(CALL_IN_PROGRESS)) {
                result.setSuccess(true);
                result.setErrorMessgae("");
                result.setReferenceId(jsonResponse.getString("reference_id"));
            }else{
	...
	private static String signString(String stringToSign) throws Throwable {
		...
		byte[] decodeSecretkeyBytes = Base64.decodeBase64(SECRET_KEY);
	...
	public static HttpClient getHttpClient(boolean useProxy) {
		...
		if (useProxy) {
			HttpHost proxy = new HttpHost("httpproxy.qa.ebay.com", 80, "http");
			// m_logger.log(LogLevel.INFO,
			// "proxyURL:"+proxyURL+", port"+port+", ProxyProtocol"+getProxyProtocol());
	...
	/**
	 * Fix "javax.net.ssl.SSLPeerUnverifiedException: peer not authenticated"
	 * issue.
	 * 
	 * @param base
	 * @return
	...
	public static HttpClient wrapClientToFixSSLIssue(HttpClient base) {
		SSLSocketFactory ssf = DefaultSSLSocketFactory.getInstance("TLSv1.2");
		try {
